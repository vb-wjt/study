...
文件 `SI4.0.1.md` 已充实备份恢复章节(2026-05-13):
...
- ~~**备份恢复**:备份哪些数据(Mongo / 文件 / 配置)?恢复时间(RTO)和数据丢失(RPO)?~~ ✅ 2026-05-13 已补充
  - 详细设计文档 `backup-restore/design/`(3 md + 8 puml + 20 png):三方案选型 + pg_basebackup 性能基准(1kw~10 亿行) + ZE 数据同步 + 跨机器恢复
  - 性能: 1.2 亿行备份 28min / 恢复 20min(8.8G);10 亿行备份 3h32m / 恢复 3h35m(56.1G)
- **MongoDB → FerretDB 迁移**:**这部分 `3-business/PI/迁移 mongodb.xmind` 有覆盖吗?还是 SI 自己有一份?**
  - [wu] 都是用的同一套

### 1.3 SI 4.1 `[必须]`

文件 `SI4.1.md` 已充实 SCID-Fath 章节(2026-05-13):
- **解析 MIB 的技术选型**:
  - 为什么选 SNMP4J-SMI-PRO 而不是 Mibble / 自研?(`resolve-mib.md` 754 行有详细方案,但**选型对比理由没明说**)
  - 商业 license 的成本和谁买单?
  - [wu] 见 resolve-mib.md 里面有选型原因，且许可证由公司付费
- ~~**SCID-Fath 是什么**?手册 / 标准里的术语?~~ ✅ 2026-05-13 已补充
  - [wu] 两款由德国 Fath公司开发和销售的智能锁，会在维谛的一个数据中心边缘统一采集器 SCID(Remote Device Unit, RDU类型)中加入监控，并提供操作
  - 详细设计文档 `SCID/SCID-DDD.md`(DDD) + `SCID/SCID-RDD.md`(需求讨论):8 个接口改动 + 数据模型 + UML 授权流程 + 5 锁类型对比 + 会议决策记录
- ~~**跨团队合作 / 联调**~~ ✅ 2026-05-13 已补充(推动国外采购 + RDU 团队协商通过 SNMP 信号区分型号 + 独立分支开发)
  - 仍需补充: 具体在哪个环节出现过冲突?怎么解决的?(面试常问)
- ~~**硬件搭建**~~ ✅ 2026-05-13 已补充(Fath 锁接入 SCID)
  - [wu] 把 Fath锁接入 SCID
